...
class VisionADClient:
    ...
    def calculate_f1_from_zip(self, zip_path: str, normal_images: List[str],
                              abnormal_images: List[str], threshold: float = 0.5) -> Tuple[Optional[Dict], Optional[str]]:
        """
        저장된 ZIP 파일에서 F1 Score 계산

        Args:
            zip_path: 배치 추론 결과 ZIP 파일 경로
            normal_images: 정상 이미지 경로 리스트 (파일명 매칭용)
            abnormal_images: 비정상 이미지 경로 리스트 (파일명 매칭용)
            threshold: 이상 판정 임계값
        ...
        try:
            if not os.path.exists(zip_path):
                return None, f"ZIP 파일을 찾을 수 없습니다: {zip_path}"
